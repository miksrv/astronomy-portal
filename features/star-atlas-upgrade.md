...
- **Time controls beyond FE-2:** a "Сегодня ночью" preset (`nightPreset.ts`: next end of astronomical twilight, degrading to nautical/civil, disabled under polar day) and an animated **time flow** (`timeFlow.ts`) — ÷8 / ÷2 / ⏸ / ×2 / ×8 in one rail, the way a phone planetarium does it, replacing the earlier ±1 h / ±1 d steppers (`timeStep.ts`, removed). `rate` is seconds of sky per real second, clamped to 1× (real time — there is no reverse and no slow motion, the ÷ buttons only undo an acceleration) … 86400× (1 day/s). Pause freezes the sky at the moment the flow reached and resets the speed; slowing back to 1× pins that moment too, rather than snapping the sky back to the real "now" and throwing the travelling away.
- **The flow never goes through React.** The simulated clock lives in `nowRef` and the rAF loop in `useCelestialDisplay` redraws the canvas straight from there, so no state changes at animation rate and the settings panel is not re-rendered by the animation; the two captions that show the moment (status chip, date field) poll `resolveDate()` on their own once-a-second tick. The clock advances every frame while the redraw paces itself at `REDRAW_HEADROOM` × the last redraw's own duration (bounded by `MIN/MAX_REDRAW_INTERVAL_MS`), so frame rate changes the smoothness, never the speed of time. Measured on `/starmap` over CDP: `Celestial.skyview()` costs ~22 ms regardless of devicePixelRatio (the cost is projection maths, not fill rate), and the flow settles at ~20 redraws/s with zero long tasks.
