...
- **BE-1 stand-in: static city list.** `client/public/data/cities.json` (~160 entries: every Russian regional centre and city over 500k, CIS/Baltic capitals, major world cities) drives a combobox "Город" in `StarMapLocationControl` (`cities.ts`: `matchCities`, `findNearestCity`). No third-party call is made (Business Rule 4 holds). When the geocode proxy lands, the same input can fall through to it for places not in the list. The status chip shows the nearest listed city within 25 km instead of raw coordinates.
...
- **Business Rule 12 in the UI.** Timezone resolution (`timezone.ts`, local polygon lookup, no network) is lazy on a bare visit but eager whenever a date is set (permalink `dt`, presets, steps); the date input is labelled "Дата и время (местное для точки)" and shows the resolved zone/offset under it.
...
- **Atmosphere toggle.** `daylight.show` is a user setting (`atmosphere`, default on, horizon mode only, live-patched) and a permalink parameter (`atm=0`), so the daytime sky can be inspected with stars visible.
- **Layout.** On desktop the settings panel is a docked 280 px sidebar that shrinks the map rather than overlaying it; on mobile it stays a bottom sheet. A Stellarium-style quick bar at the bottom toggles constellation lines/names, graticule, DSOs, planets, Milky Way, meteor radiants, horizon mode and atmosphere. One left-hand rail holds zoom in/out, "fit view" (re-centres the zenith and re-fits the horizon circle, or resets zoom in sky mode) and the settings/search/link buttons; d3-celestial's own zoom buttons are disabled on `/starmap` only (`controls: false`), object/photo pages keep them. The mobile FAB toggles horizon/sky mode.
...
- [~] A user can search a city by name and the map re-centers/re-computes for that location — **partially**: works today against the bundled `cities.json` list (no third-party call); the `GET /geocode` proxy for arbitrary places is still deferred (BE-1)
