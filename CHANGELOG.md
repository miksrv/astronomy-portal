...
- Reworked stargazing event cards from a 3-per-row tile grid into a single-row layout, unifying `/stargazing`, `/stargazing/history`, and the `/profile` event history into one shared `EventRow` component (`variant="public" | "personal"`) — the row now also surfaces the event's location and a short plain-text excerpt of its description next to the location, with the view/member counts shown inline as icon+count separated by `•`; the excerpt is generated by a new backend `EventsModel::excerptFromMarkdown()`, which strips markdown syntax and truncates on a word boundary, backing a new `excerpt` field on the events list response (`GET /events`) alongside the previously omitted `location` column; the now-redundant `event-meta-row` component and the old tile grid CSS were removed
- Had `/profile` event history reuse the same `EventRow` (via a new `userId` filter on `GET /events`) instead of its own list markup, and gave every event card a viewer-scoped "Посещено"/"Отзыв оставлен" (green) or "Нет отзыва" (red) badge, backed by new bulk booking/review lookups so the flags are attached in one query instead of one per event
- Added year grouping to `/stargazing/history`: events are now split into per-year sections with a continuous timeline marker (dot + year label, connected by a vertical line) via a new `groupByYear` prop on `EventsList`, left off (unchanged behavior) on `/stargazing` and the `/profile` event history
- UI/UX improvements for the stargazing events list
