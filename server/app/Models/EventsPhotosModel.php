...
class EventsPhotosModel extends ApplicationBaseModel
{
     ...
     * $order is 'rand' the rows are returned in random order; every other
     * case (including no $order at all, or an explicit 'date') sorts by
     * capture date ascending — falling back to the upload time for photos
     * with no EXIF timestamp — so the gallery reads in the order the photos
     * were actually taken, not the order they happened to be uploaded in.
     * The $limit is capped at MAX_LIMIT for safety, regardless of whether
     * the list is scoped to one event.
     *
