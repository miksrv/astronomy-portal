 ...
 * EventsMediaModel
 *
 * Manages the `events_media` table, which stores photos and videos uploaded
 * by users as part of a specific stargazing event (FEAT-26 renamed this
 * from `events_photos`/EventsPhotosModel once video joined the gallery — see
 * features/stargazing-event-video-uploads.md). Supports soft deletes and
 * UUID PKs.
 ...
 */
class EventsMediaModel extends ApplicationBaseModel
{
    protected $table            = 'events_media';
    protected $primaryKey       = 'id';
    protected $useAutoIncrement = false;
    protected $returnType       = EventMediaEntity::class;
    ...
    /**
     * Retrieves a paginated and optionally ordered list of event media
     * (photos and videos, mixed in one chronological feed).
     *
     ...
     * upload time for items with no EXIF/client-supplied timestamp — so the
     * gallery reads in the order the media was actually taken, not the order
     * it happened to be uploaded in. The $limit is capped at MAX_LIMIT for
     ...
     * @return array Array of EventMediaEntity objects.
     */
    public function getMediaList(
        ...
        $mediaQuery = $this->select('id, event_id, media_type, photographer_name, taken_at, file_name, file_ext, width, height, duration');

        if ($eventId) {
            $mediaQuery->where('event_id', $eventId);
        }

        if ($photographer) {
            $mediaQuery->where('photographer_name', $photographer);
        }
        ...
        // media of an event with e.g. 150 items would silently get 20 back.
        $mediaQuery->limit(
        ...
        if ($order === 'rand') {
            $mediaQuery->orderBy('RAND()');
        } else {
            // Capture date ascending, falling back to upload time when the
            // item has no EXIF/client-supplied timestamp. $escape must be
            // explicitly false here - CI4's orderBy() otherwise splits the
            // raw expression on its internal comma (treating it as multiple
            // order columns) and mangles it into invalid SQL:
            // `COALESCE(taken_at ASC, created_at) ASC`.
            $mediaQuery->orderBy('COALESCE(taken_at, created_at)', 'ASC', false);
        }

        $mediaList = $mediaQuery->findAll();

        return $mediaList ?: [];
    }

    /**
     * Counts event media matching the same $eventId/$photographer scoping as
     * {@see getMediaList()}, ignoring pagination — lets clients know the real
     ...
     */
    public function countMediaList(?string $eventId = null, ?string $photographer = null): int
    {
        if ($eventId) {
     ...
     * autocomplete suggestions — independent of which page of media happens
     * to be loaded, so a photographer whose items only appear past the first
