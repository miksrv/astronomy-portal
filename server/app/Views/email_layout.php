<?php
/**
 * Shared HTML shell for all outgoing emails — head/CSS, the 600px centered
 * table layout, and the brand footer (site tagline + link row). Callers
 * render their own body markup into `$content` and include this view; pass
 * `$unsubscribeUrl` only for mailing campaigns (`email_newsletter.php`) —
 * transactional emails like `email_ticket.php` omit it, which also hides the
 * "you're receiving this because you're subscribed" line, since a ticket
 * confirmation isn't a mailing subscription.
 */
?>
<!doctype html>
<html lang="<?= $locale ?? 'ru' ?>">
<head>
    ...
    <style media="all" type="text/css">
        ...
        /* INLINE IMAGE (newsletter header image) */
        .newsletter-image {
            border-radius: 8px;
            display: block;
            margin-bottom: 24px;
                ...
                <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" class="footer">
                    <tr>
                        <td style="border-radius: 0 0 8px 8px; padding: 0px; text-align: center;">
                            ...
                            <p style="color: #656d76; font-size: 12px; margin: <?= !empty($unsubscribeUrl) ? '0 0 0px' : '0' ?>; font-family: Helvetica, Arial, sans-serif;">
